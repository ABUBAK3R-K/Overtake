...
def run_monte_carlo(
    ...
    for sim_idx in range(n_sims):
        # Driver state tracking for this trajectory
        sim_tyres = {d: list(state.tyres.get(d, ("MEDIUM", 1))) for d in drivers}
        # Accumulated race time relative to leader at start of simulation
        sim_gaps = {d: float(state.gaps.get(d, 0.0)) for d in drivers}
        driver_trajectory = []
        ...
        for sim_lap in range(state.lap + 1, state.total_laps + 1):
            ...
            is_sc = np.random.random() < sc_prob

            lap_times_this_sim = {}
            for d in drivers:
                comp, age = sim_tyres[d]
                is_target = (d == target_driver)

                # Pit stop logic
                did_pit = False
                if is_target and sim_lap in pit_plan:
                    # Target driver planned pit stop
                    comp = pit_plan[sim_lap]
                    age = 0
                    did_pit = True
                elif not is_target:
                    # Rival dynamic pit stop logic: pit if tyre is past cliff or under SC opportunity
                    max_life = COMPOUND_MAX_LIFESPAN.get(comp, 30)
                    pit_chance = 0.0
                    if age >= max_life:
                        pit_chance = 0.65
                    elif is_sc and age >= max_life * 0.6:
                        pit_chance = 0.40  # opportunistic stop under SC

                    if np.random.random() < pit_chance:
                        comp = "HARD" if comp == "MEDIUM" else "MEDIUM"
                        age = 0
                        did_pit = True

                sim_tyres[d] = [comp, age + 1]

                # Base lap time estimation
                base_pace = driver_base_paces[d]
                try:
                    wear = predict_tyre_degradation(
                        comp, age, state.circuit,
                ...
                lap_time = base_pace + wear + noise

                if is_sc:
                    lap_time *= 1.537  # SC pace delta multiplier

                if did_pit:
                    pit_loss = PIT_LOSS_SAFETY_CAR if is_sc else PIT_LOSS_RACING
                    lap_time += pit_loss + np.random.normal(0.0, 0.5)

                sim_gaps[d] += lap_time

            # Leader of the simulation lap has minimum accumulated time
            min_time = min(sim_gaps.values())
            for d in drivers:
                sim_gaps[d] -= min_time
            ...
            if is_sc:
                # Rank and compress
                sorted_d = sorted(drivers, key=lambda d: sim_gaps[d])
                for rank, d in enumerate(sorted_d):
                    sim_gaps[d] = rank * 0.85 + np.random.uniform(0.0, 0.2)

            # Record target driver rank on this lap
            ranked = sorted(drivers, key=lambda d: sim_gaps[d])
            target_pos = ranked.index(target_driver) + 1
            if sim_idx < 10:  # store sample paths for top 10 sims
                driver_trajectory.append(target_pos)

        # Record final simulation outcome
        final_ranks = sorted(drivers, key=lambda d: sim_gaps[d])
        final_pos = final_ranks.index(target_driver) + 1
        finish_positions.append(final_pos)
        finish_times.append(sim_gaps[target_driver])
        if sim_idx < 10:
            sample_paths.append(driver_trajectory)
